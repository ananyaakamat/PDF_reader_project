...
server.tool(
  "read-pdf",
  {
    ...
    pages: z
      .string()
      .optional()
      .describe("Page range (e.g., '1-5', '1,3,5', 'all'). Default: 'all'"),
    include_metadata: z
      .boolean()
      .optional()
      .describe("Include PDF metadata in output. Default: true"),
    clean_text: z
      .boolean()
      .optional()
      .describe("Clean and normalize extracted text. Default: false"),
  },
  async ({
    file,
    pages = "all",
...
server.tool(
  ...
  async ({ file, query, case_sensitive = false, whole_word = false }) => {
    try {
      ...
      let response = `Search results for "${query}" in ${path.basename(
        file
      )}:\n`;
      response += `Found ${results.length} matching lines with ${results.reduce(
        (sum, r) => sum + r.matches,
        0
      )} total matches\n\n`;
